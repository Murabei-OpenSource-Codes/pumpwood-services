...
describe("DeleteService", () => {
    ...
    it("should return success on successful deletion", async () => {
        ...
        const [data, error] = await (0, __1.DeleteService)(api, "activities", 1);
        expect(error).toBeNull();
        expect(data).toEqual({ deleted: true });
        expect(fetchMock).toHaveBeenCalledWith("https://example.com/activities/delete/1/", expect.objectContaining({
            method: "DELETE",
    ...
    it("should return response data when API returns additional content", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://example.com/activities/delete/1/", expect.anything());
    });
    it("should return error on failure", async () => {
        fetchMock.mockResponseOnce("Not Found", { status: 404 });
        const [data, error] = await (0, __1.DeleteService)(api, "activities", 999);
    ...
    it("should handle server errors correctly", async () => {
        ...
        const [data, error] = await (0, __1.DeleteService)(api, "users", 1);
        expect(data).toBeNull();
        expect(error).toBeInstanceOf(Error);
